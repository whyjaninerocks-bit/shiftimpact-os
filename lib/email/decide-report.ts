...
const CONTENT: Record<string, CategoryContent> = {
  ...
  stop: {
    ...
    benchmarks: [
      { title: "Signals required for a confident stop decision", figure: "3 or more", context: "The typical standard for a stop decision in campaigns above RM50,000 in SEA markets, with at least three independent signals all pointing to underperformance.", position: "How many signals are you reading?", posClass: "signal-amber" },
...
// Traffic light system only: green = Press, amber = Hold / Pivot / Investigate, red = Stop
function postureColour(posture: string): { bg: string; text: string; border: string } {
  ...
  // Hold, Pivot, Investigate all map to amber (caution)
