  ...
  return (
    <div className="space-y-4">
      ...
      <form action={upsertAction} className="space-y-5">
        ...
        <div className="border border-teal-100 rounded-md p-3 bg-teal-50/30">
          <p className="text-xs font-semibold text-teal-800 mb-1">
            Signal 3B — Video Completion Rate / VCR (Demand — optional)
          </p>
          <p className="text-xs text-teal-600 mb-3">
            TikTok-native. % of viewers who watch your video to completion. Strong supplementary Demand signal — if VCR is above target it is included in Gate assessment alongside the primary four. Source: TikTok Business dashboard.
          </p>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className={labelClass}>Green threshold (%)</label>
              <input
                type="number" name="signal_3b_target_pct" className={inputClass} step="0.1"
                defaultValue={threshold?.signal_3b_target_pct ?? 70} disabled={locked}
              />
              <p className="text-xs text-neutral-400 mt-0.5">VCR % = Green</p>
            </div>
            <div>
              <label className={labelClass}>Amber threshold (%)</label>
              <input
                type="number" name="signal_3b_amber_pct" className={inputClass} step="0.1"
                defaultValue={threshold?.signal_3b_amber_pct ?? 50} disabled={locked}
              />
              <p className="text-xs text-neutral-400 mt-0.5">Approaching = Amber</p>
            </div>
            <div>
              <label className={labelClass}>Red threshold (%)</label>
              <input
                type="number" name="signal_3b_red_pct" className={inputClass} step="0.1"
                defaultValue={threshold?.signal_3b_red_pct ?? 30} disabled={locked}
              />
              <p className="text-xs text-neutral-400 mt-0.5">At/below = Red</p>
            </div>
          </div>
        ...
        {/* Signal 4 — Retention */}
        <div className="border border-violet-100 rounded-md p-3 bg-violet-50/30">
          <p className="text-xs font-semibold text-violet-800 mb-1">
            Signal 4 — Retention / Repeat Visit Rate (LAG — optional)
          </p>
          <p className="text-xs text-violet-600 mb-3">
            % of campaign audience who return within the campaign window. LAG signal — confirms what already happened, not what's coming. Used for diagnosis only, not Gate. GrabAds API is the upgrade path; manual entry for now.
          </p>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className={labelClass}>Green threshold (%)</label>
              <input
                type="number" name="signal_4_target_pct" className={inputClass} step="0.1"
                defaultValue={threshold?.signal_4_target_pct ?? 15} disabled={locked}
              />
            </div>
            <div>
              <label className={labelClass}>Amber threshold (%)</label>
              <input
                type="number" name="signal_4_amber_pct" className={inputClass} step="0.1"
                defaultValue={threshold?.signal_4_amber_pct ?? 8} disabled={locked}
              />
            </div>
            <div>
              <label className={labelClass}>Red threshold (%)</label>
              <input
                type="number" name="signal_4_red_pct" className={inputClass} step="0.1"
                defaultValue={threshold?.signal_4_red_pct ?? 3} disabled={locked}
              />
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <select
            name="market_code"
            className={inputClass}
            defaultValue={threshold?.market_code ?? "MY"}
            disabled={locked}
          >
            <option value="MY">Malaysia (MY) — TikTok Save+Share primary</option>
            <option value="SG">Singapore (SG) — Share of Search primary</option>
            <option value="PH">Philippines (PH) — Facebook Group Mentions primary</option>
            <option value="TH">Thailand (TH) — TikTok Sound Adoption primary</option>
            <option value="ID">Indonesia (ID) — TikTok UGC Volume primary</option>
          </select>
          <p className="text-xs text-neutral-400 mt-1">
            Tells the AI which signals carry the most weight in this market for its recommendations.
          </p>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        >
          ...
          {/* Optional signals — Signal 3B (VCR) + Signal 4 (Retention) */}
          {(threshold.signal_3b_target_pct || threshold.signal_4_target_pct) && (
            <div className="border-t border-neutral-100 pt-4 space-y-3">
              <p className="text-xs font-medium text-neutral-500">Optional signals (enter if available)</p>
              <div className="grid sm:grid-cols-2 gap-4">
                {threshold.signal_3b_target_pct && (
                  <div>
                    <label className={labelClass}>Signal 3B — VCR (%)</label>
                    <input
                      type="number" name="signal_3b_actual_pct" className={inputClass}
                      step="0.1" placeholder={`Target ≥${threshold.signal_3b_target_pct}%`}
                    />
                    <p className="text-xs text-teal-600 mt-0.5">Demand — video completion rate (TikTok)</p>
                  </div>
                )}
                {threshold.signal_4_target_pct && (
                  <div>
                    <label className={labelClass}>Signal 4 — Retention (%)</label>
                    <input
                      type="number" name="signal_4_actual_pct" className={inputClass}
                      step="0.1" placeholder={`Target ≥${threshold.signal_4_target_pct}%`}
                    />
                    <p className="text-xs text-violet-600 mt-0.5">LAG — repeat visit rate (diagnosis only)</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="space-y-4">
          ...
          {activeWeek && (
            <div className="space-y-4">
              ...
              {/* Optional Signal 3B (VCR) + Signal 4 (Retention) traffic lights */}
              {!activeWeek.flags_suppressed &&
                (activeWeek.signal_3b_health || activeWeek.signal_4_health) && (
                  <div className="flex gap-3 flex-wrap">
                    {activeWeek.signal_3b_health && (
                      <div className={`flex flex-col items-center gap-1.5 p-3 rounded-lg border ${
                        activeWeek.signal_3b_health === "Green"
                          ? "bg-emerald-50 border-emerald-200"
                          : activeWeek.signal_3b_health === "Amber"
                        ...
                        <p className={`text-xs font-bold ${HEALTH_STYLE[activeWeek.signal_3b_health as SignalHealth].text}`}>
                          VCR
                        </p>
                        <p className="text-xs text-neutral-400 text-center leading-tight">Signal 3B</p>
                        <span className="text-xs text-teal-600 bg-teal-50 border border-teal-200 px-1.5 py-0.5 rounded-full">
                          supplementary
                        </span>
                      </div>
                    )}
                    {activeWeek.signal_4_health && (
                      <div className={`flex flex-col items-center gap-1.5 p-3 rounded-lg border ${
                        activeWeek.signal_4_health === "Green"
                          ? "bg-emerald-50 border-emerald-200"
                          : activeWeek.signal_4_health === "Amber"
                          ? "bg-amber-50 border-amber-200"
                          : "bg-red-50 border-red-200"
                      }`}>
                        <div className="flex flex-col gap-1">
                        ...
                        <p className={`text-xs font-bold ${HEALTH_STYLE[activeWeek.signal_4_health as SignalHealth].text}`}>
                          Retention
                        </p>
                        <p className="text-xs text-neutral-400 text-center leading-tight">Signal 4 (LAG)</p>
                        <span className="text-xs text-violet-600 bg-violet-50 border border-violet-200 px-1.5 py-0.5 rounded-full">
                          diagnosis only
                        </span>
                      </div>
                    )}
