...
SALES QUALITY READ:
Not a second scoring pass. State only the pattern the five layers above already support — brand_led, promotion_led, platform_spike, discount_trained, conversion_blocked, repeatable_demand, or competitor_leakage. If the five layers do not clearly support any named pattern, say insufficient_evidence. Every pattern you name must be traceable in your rationale to specific layer evidence stated above it. Internal consistency rule: if your own rationale states that movement into a purchase action is not publicly visible, or that the trail goes cold before a purchase, you may not select brand_led, commerce_mover-shaped, or repeatable_demand language for the pattern — select conversion_blocked instead. A pattern label must never contradict what the rationale sentence right next to it says.
...
This must be internally consistent with leakage_pattern and sales_quality_read — never pick a classification that contradicts the leakage location you just named. Specific binding rules, applied in order:
1. Use commerce_mover ONLY when public evidence shows demand is credibly moving into a purchase path, a commerce action, or a measurable conversion proxy — an actual observed step toward purchase, not merely the possibility of one. Commerce access existing, or product interest existing, is never sufficient on its own to justify commerce_mover.
2. If leakage_pattern.location names the trail going cold at route to purchase, product assignment, purchase, or repeat — meaning the break is downstream of proof, at the handoff into buying — final_classification must be conversion_blocked, not commerce_mover, regardless of how strong Brand Power or Product Conviction are.
3. If Commerce Capture is rated available but not decisive, and the leakage location is specifically about a missing handoff, missing purchase path, or missing route to purchase, final_classification must be conversion_blocked.
4. The canonical pattern: strong Brand Power, real Product Conviction, available Commerce Capture, and no visible public movement into a specific product choice or completed purchase path is conversion_blocked, never commerce_mover — strong upstream layers describe an opportunity being generated, not a sale being closed, and classification tracks whether the sale closes.
5. Only classify commerce_mover when the leakage_pattern location itself sits upstream of purchase (for example at brand power, product conviction, or proof) while the public trail simultaneously shows real evidence of purchase-path movement elsewhere — a case where demand generation is the weaker link, not commerce completion.
Before finalizing, check final_classification against leakage_pattern.location and sales_quality_read.pattern in that order — if any of the three could be read as contradicting another, resolve the conflict in favor of the more conservative (blocked/risk-leaning) classification, never the more commercially flattering one.
