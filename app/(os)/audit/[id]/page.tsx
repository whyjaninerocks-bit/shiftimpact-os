...
// Detects the model's honest "no defensible benchmark" response so the UI can
// render it distinctly (muted/italic) from a real, sourced benchmark claim —
// see BENCHMARK DISCIPLINE rule in app/api/audit-analyze/route.ts. Matched
// loosely (case-insensitive substring) since the model may vary punctuation
// slightly despite the exact-string instruction in the prompt.
function isUnsourcedBenchmark(text: string | null | undefined): boolean {
  if (!text) return false;
  return /no defensible public benchmark/i.test(text);
}
...
// ─── Prospect-Stage Brand-Commerce Read (stage_marker: "prospect_preview") ────
//
// Generated by the corrected app/api/audit-analyze/route.ts prompt. Renders
// through ProspectPreviewReportView below instead of the legacy Campaign
// Intelligence Preview layout — see the stage_marker branch in
// AuditReportPage. Legacy rows (no stage_marker) are completely untouched by
// anything in this section and keep rendering through the original layout.

type EvidenceConfidence = "direct_evidence" | "public_proxy" | "inference" | "public_nonvisibility" | "insufficient_evidence";

type LayerRead = { rating: string; evidence_confidence: EvidenceConfidence; notes: string };

type ProspectPreviewResult = {
  stage_marker: "prospect_preview";
  campaign_phase: string;
  priority_context_note: string | null;
  estimated_campaign_week: string;
  five_layer_read: {
    brand_power: LayerRead;
    product_conviction: LayerRead;
    commerce_capture: LayerRead;
    competitor_capture: LayerRead;
    promotion_dependency: LayerRead;
  };
  sales_quality_read: { pattern: string; rationale: string };
  leakage_pattern: { location: string; confidence: EvidenceConfidence };
  final_classification: string;
  consumer_state: { stage_n: number; stage_name: string; state_transition_risk: string; note: string };
  provisional_ics: {
    applicable: boolean;
...
type QuickAuditProspectPreview = {
  id: string;
  brand_name: string;
  campaign_name: string;
  industry: string;
  ...
  business_objectives: string[] | null;
  channels: string[] | null;
  result: ProspectPreviewResult;
  created_at: string;
};

const CLASSIFICATION_META: Record<string, { label: string; color: string; ringColor: string }> = {
  brand_builder:         { label: "Builder",     color: "text-emerald-400", ringColor: "#34d399" },
  commerce_mover:         { label: "Mover",       color: "text-emerald-400", ringColor: "#34d399" },
  promo_extractor:        { label: "Extractor",   color: "text-amber-400",   ringColor: "#fbbf24" },
  brand_risk:             { label: "Risk",        color: "text-red-400",     ringColor: "#f87171" },
  inefficient_activity:   { label: "Inefficient", color: "text-amber-400",   ringColor: "#fbbf24" },
  conversion_blocked:     { label: "Blocked",     color: "text-red-400",     ringColor: "#f87171" },
};

function classificationMeta(c: string) {
  return CLASSIFICATION_META[c] ?? { label: "Unclear", color: "text-slate-400", ringColor: "#94a3b8" };
}

function ClassificationRing({ classification }: { classification: string }) {
  ...
  return (
    <div className="relative shrink-0 w-14 h-14">
      <svg width="56" height="56" viewBox="0 0 56 56" style={{ transform: "rotate(-90deg)" }}>
        <circle cx="28" cy="28" r={r} fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth="5" />
        <circle cx="28" cy="28" r={r} fill="none" stroke={meta.ringColor} strokeWidth="5"
          strokeDasharray={`${circ} ${circ}`} strokeLinecap="round" />
      </svg>
      <div className="absolute inset-0 flex items-center justify-center px-1">
        <span className="text-[8px] font-bold text-white leading-none text-center">{meta.label}</span>
      </div>
    </div>
  );
}

// Bar fill is discrete (25/55/85/0), never a continuous percentage — a
// continuous width would silently reintroduce false precision even without
// a number printed on screen. promotion_dependency is inverted (high = risk,
// not strength), everything else reads strong = good.
function layerVisual(key: string, rating: string): { width: number; color: string } {
  if (key === "promotion_dependency") {
    if (rating === "high") return { width: 85, color: "bg-red-400" };
    if (rating === "moderate") return { width: 55, color: "bg-amber-400" };
    if (rating === "low") return { width: 25, color: "bg-emerald-400" };
    return { width: 0, color: "bg-slate-300" }; // unproven
  }
  if (rating === "strong") return { width: 85, color: "bg-emerald-400" };
  if (rating === "moderate") return { width: 55, color: "bg-amber-400" };
  if (rating === "weak") return { width: 25, color: "bg-red-400" };
  return { width: 0, color: "bg-slate-300" }; // unclear
}

function gateIndicatorStyle(status: string) {
  if (status === "worth_a_conversation_now") return { bg: "bg-red-500", text: "text-white", label: "Worth a Conversation" };
  if (status === "worth_watching")           return { bg: "bg-amber-500", text: "text-white", label: "Worth Watching" };
  return { bg: "bg-slate-600", text: "text-white", label: "Not Yet Urgent" };
}

// public_status is never rendered as a pass/fail check — not_publicly_visible
// must read distinctly from a failed check, per the evidence-confidence
// correction (public nonvisibility is not proof of absence).
function publicStatusIcon(status: string) {
  if (status === "observed") return { icon: "✓", cls: "text-emerald-600", bg: "bg-emerald-50 border-emerald-100", caption: null as string | null };
  if (status === "not_publicly_visible") return { icon: "○", cls: "text-slate-400", bg: "bg-slate-50 border-slate-100", caption: "Not publicly visible — not the same as absent" };
  return { icon: "–", cls: "text-slate-300", bg: "bg-slate-50 border-slate-100", caption: "Unclear from public sources" };
}

function icsBandBadge(band: string) {
  if (band === "strong")   return "bg-emerald-50 border-emerald-200 text-emerald-700";
  if (band === "moderate") return "bg-amber-50 border-amber-200 text-amber-700";
  if (band === "weak")     return "bg-red-50 border-red-200 text-red-700";
...
function ProspectPreviewReportView({ audit }: { audit: QuickAuditProspectPreview }) {
  ...
  const classMeta = classificationMeta(r.final_classification);
  const gate = gateIndicatorStyle(r.prospect_gate_indicator.status);

  const layers: { key: keyof ProspectPreviewResult["five_layer_read"]; label: string }[] = [
    { key: "brand_power", label: "Brand Power" },
    { key: "product_conviction", label: "Product Conviction" },
    { key: "commerce_capture", label: "Commerce Capture" },
    { key: "competitor_capture", label: "Competitor Capture" },
    { key: "promotion_dependency", label: "Promotion Dependency" },
  ];
    ...
    <div className="-mx-4 sm:-mx-6 -mt-8 bg-slate-50 min-h-screen">
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-6 sm:pt-8 pb-7">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="flex items-center gap-3 px-4 py-3.5 rounded-xl"
              ...
              <ClassificationRing classification={r.final_classification} />
              <div>
                <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wide mb-0.5">Read</p>
                <p className={`text-base font-bold ${classMeta.color}`}>{classMeta.label}</p>
              </div>
            </div>

            <div className="px-4 py-3.5 rounded-xl"
              style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)" }}>
              <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wide mb-2.5">Five-Layer Read</p>
              <div className="space-y-2">
                {layers.map(({ key, label }) => {
                  const layer = r.five_layer_read[key];
                  const visual = layerVisual(key, layer.rating);
                  return (
                    <div key={key}>
                      ...
                      <div className="h-1.5 rounded-full overflow-hidden"
                        ...
                        <div className={`${visual.color} h-1.5 rounded-full`} style={{ width: `${visual.width}%` }} />
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="flex flex-col items-center justify-center gap-2 px-4 py-3.5 rounded-xl"
              style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)" }}>
              <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wide">Indicative Read</p>
              <span className={`text-sm font-bold px-4 py-2 rounded-xl text-center ${gate.bg} ${gate.text}`}>
                {gate.label}
              </span>
            </div>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-7 space-y-5">
        ...
        <div className="rounded-xl border border-slate-200 bg-white px-4 sm:px-6 py-5 shadow-sm">
          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-3">Brand-Commerce Read</p>
          <blockquote className="border-l-[3px] border-slate-900 pl-4 text-base font-semibold text-slate-900 leading-snug mb-4">
            {classMeta.label}: {r.leakage_pattern.location}
          </blockquote>
        ...
        <div data-pdf-break="before" className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="px-6 pt-5 pb-4 border-b border-slate-100">
            ...
            <p className="text-xs text-slate-400 mt-1">Public checks against the leakage pattern above</p>
          </div>
          <div className="px-6 pb-5 pt-4 space-y-2">
            {r.prospect_gate_indicator.leakage_conditions.map((c, i) => {
              const tick = publicStatusIcon(c.public_status);
              return (
                <div key={i} className={`flex items-start gap-3 rounded-lg px-4 py-3 border ${tick.bg}`}>
                  <span className={`text-sm font-bold shrink-0 mt-0.5 ${tick.cls}`}>{tick.icon}</span>
                  <div>
                    <p className="text-xs font-semibold text-slate-700">{c.condition}</p>
                    <p className="text-[10px] text-slate-500 mt-0.5 leading-relaxed">{c.evidence}</p>
                    {tick.caption && <p className="text-[9px] text-slate-400 italic mt-1">{tick.caption}</p>}
                  </div>
                </div>
              );
            })}
        ...
        <div data-pdf-break="before" className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="px-6 pt-5 pb-4 border-b border-slate-100">
            <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Consumer Behaviour Stage</p>
          </div>
          <div className="px-6 py-5">
            <StrategicPOV
              header={`${r.consumer_state.stage_name} · ${r.consumer_state.state_transition_risk} Transition Watch`}
              body={r.consumer_state.note}
            />
            <ConsumerStateArc currentState={r.consumer_state.stage_n} />
          </div>
        </div>

        {r.provisional_ics.applicable && r.provisional_ics.estimated_band !== "not_applicable" && (
          <div data-pdf-break="before" className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            <div className="px-6 pt-5 pb-4 border-b border-slate-100">
              <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Idea Certainty — Provisional Read</p>
            </div>
            <div className="px-6 py-5">
              <span className={`text-xs font-semibold border px-3 py-1 rounded-full capitalize ${icsBandBadge(r.provisional_ics.estimated_band)}`}>
                {r.provisional_ics.estimated_band}
              </span>
        ...
        <div data-pdf-break="before" className="rounded-xl border border-slate-200 bg-white shadow-sm px-6 py-7">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">
            The Question This Read Can&rsquo;t Answer
          </p>
          <blockquote className="border-l-[3px] border-slate-900 pl-4 text-sm font-semibold text-slate-900 leading-snug mb-4">
          ...
          <div className="flex items-center justify-between mt-5 pt-4 border-t border-slate-100">
            ...
            <span className="text-[10px] text-slate-400">
              Confidential · Brand-Commerce Prospect Read
            </span>
          </div>
        </div>
        ...
        <div data-pdf-break="before" className="rounded-xl border border-slate-200 bg-slate-50 p-5">
          ...
          <div className="space-y-2 text-[11px] text-slate-400 leading-relaxed">
            <p>
              <span className="font-semibold text-slate-500">What this read is based on:</span>{" "}
              Publicly accessible signals only — brand websites, public social media posts, Facebook Ad Library, press coverage, and KOL content available without authentication at the time of analysis.
            </p>
            <p>
              <span className="font-semibold text-slate-500">Interpretation:</span>{" "}
              This is a prospect-stage hypothesis built from public signals, not a confirmed diagnosis. Where a layer above shows lower confidence, that reflects the limits of public evidence, not a weaker finding. Confirming or disproving this read requires client data — see the checks above for exactly what.
            </p>
          </div>
        </div>
...
export default async function AuditReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // stage_marker branch — legacy quick_audits rows (generated before this
  // rebuild) have no stage_marker and render through the original Campaign
  // Intelligence Preview layout below, completely untouched. Rows generated
  // by the corrected audit-analyze prompt carry stage_marker:
  // "prospect_preview" and render through ProspectPreviewReportView instead.
  if (raw.result?.stage_marker === "prospect_preview") {
    const prospectAudit: QuickAuditProspectPreview = {
      id: raw.id,
      brand_name: raw.brand_name,
      campaign_name: raw.campaign_name,
      industry: raw.industry,
      campaign_phases: raw.campaign_phases,
      business_objectives: raw.business_objectives,
                  ...
                  <div className="ml-[52px]">
                    ...
                    <div className="px-3 py-2.5 bg-slate-50 rounded-lg border border-slate-100 space-y-1.5">
                      <div className="flex items-start gap-1.5">
                        <span className="text-xs text-slate-400 font-medium shrink-0">Benchmark:</span>
                        {isUnsourcedBenchmark(sig.benchmark_context) ? (
                          <span className="text-xs text-slate-400 italic">No defensible public benchmark available for this signal in this market.</span>
                        ) : (
