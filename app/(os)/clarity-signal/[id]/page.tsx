...
function statusConfig(status: string) {
  switch (status) {
    case "Ready":
      return { dot: "bg-emerald-500", badge: "bg-emerald-50 text-emerald-700 border-emerald-200", label: "Ready" };
    case "Watch":
      return { dot: "bg-amber-400", badge: "bg-amber-50 text-amber-700 border-amber-200", label: "Watch" };
    case "Attention":
      return { dot: "bg-orange-500", badge: "bg-orange-50 text-orange-700 border-orange-200", label: "Attention" };
    case "Intervention":
      return { dot: "bg-red-500", badge: "bg-red-50 text-red-700 border-red-200", label: "Intervention Required" };
    default:
      return { dot: "bg-neutral-400", badge: "bg-neutral-50 text-neutral-700 border-neutral-200", label: status };
  }
}

function signalStatusStyle(status: string) {
  const s = status.toLowerCase();
  if (s.includes("hold") || s.includes("at risk") || s.includes("declin") || s.includes("absent") || s.includes("weak")) {
    return "text-red-600 bg-red-50 border-red-200";
  }
  if (s.includes("pass") || s.includes("watch") || s.includes("below") || s.includes("moder")) {
    return "text-amber-700 bg-amber-50 border-amber-200";
  }
  if (s.includes("active") || s.includes("strong") || s.includes("expand") || s.includes("hold")) {
...
export default async function ClaritySignalOutputPage({
  ...
  // Guard: only render this page for clarity signal results
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-neutral-100 p-8 mb-6 shadow-sm">
        ...
        <h2 className="text-lg font-bold text-neutral-900 mb-5">Top 5 Signals</h2>

        <div className="space-y-0 divide-y divide-neutral-100">
          {(r.top_signals ?? []).slice(0, 5).map((sig, i) => (
            <div key={i} className="py-4 grid grid-cols-[1fr_auto] gap-4 items-start">
              <div>
                <p className="text-sm font-semibold text-neutral-900 mb-1">{sig.signal}</p>
                <p className="text-sm text-neutral-500 leading-relaxed">{sig.business_read}</p>
              </div>
              <span className={`text-xs font-bold px-2.5 py-1 rounded-full border whitespace-nowrap mt-0.5 ${signalStatusStyle(sig.status)}`}>
                {sig.status}
              </span>
            </div>
          ))}
        ...
        <div className="space-y-3">
          {(r.questions_worth_asking ?? []).map((q, i) => (
            <div key={i} className="flex items-start gap-3">
              <span className="text-xs font-bold text-neutral-300 mt-0.5 pt-px flex-shrink-0 w-4">{i + 1}</span>
