...
export default function DecidePage() {
  ...
  const [probeStep, setProbeStep] = useState(0); // 0-based (0=probe1, 1=probe2, 2=probe3)
  ...
  async function submitAnswer() {
    ...
    const isLast = probeStep === 2;
    setPhase("fetching");

    try {
      if (isLast) {
        // Get synthesis
        const result = await callProbe(decision.trim(), updatedConv, 3, "synthesis") as SynthesisResult;
        setSynthesis(result);
